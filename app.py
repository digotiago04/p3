...
# Seção P3 (nomes reais das abas do Sheets)
ABA_P3_DETERMINACOES_SHEET = "ORIENTACOES"  # botão "DETERMINAÇÕES"
...
/* centralizar tabelas (st.dataframe) */
...
/* tabela HTML (EVENTOS): quebra de linha real */
.table-wrap {max-height: 650px; overflow-y: auto; border: 1px solid #e5e5e5; border-radius: 8px;}
.tbl {width: 100%; border-collapse: collapse; table-layout: fixed;}
.tbl th, .tbl td {border-bottom: 1px solid #f0f0f0; padding: 10px 8px; vertical-align: top;}
.tbl th {background: #fafafa; font-weight: 700;}
.tbl td {white-space: normal; word-break: break-word;}
.tbl td.center, .tbl th.center {text-align: center;}
.tbl td.left, .tbl th.left {text-align: left;}
.rowprio {background: #90CAF9;}
</style>
""",
...
def _remove_acentos_upper(s: str) -> str:
    s = s.upper()
    return (
        s.replace("Á", "A").replace("Ã", "A").replace("Â", "A")
...
def _render_html_table(df_disp: pd.DataFrame, highlight_mask: pd.Series | None, wrap_left_cols: set[str]):
    cols = [str(c).strip().upper() for c in df_disp.columns]
    df_disp = df_disp.copy()
    df_disp.columns = cols

    if highlight_mask is None:
        highlight_mask = pd.Series([False] * len(df_disp))
    ...
    for c in cols:
        cls = "center"
        if c in wrap_left_cols:
            cls = "left"
        ths.append(f'<th class="{cls}">{esc(c)}</th>')
    thead = "<tr>" + "".join(ths) + "</tr>"

    trs = []
    for i, row in enumerate(df_disp.itertuples(index=False, name=None)):
        row_cls = "rowprio" if bool(highlight_mask.iloc[i]) else ""
        tds = []
        for c, val in zip(cols, row):
            cls = "center"
            if c in wrap_left_cols:
                cls = "left"
            tds.append(f'<td class="{cls}">{esc(val)}</td>')
...
# ==========================================================
# EXIBIÇÕES (por mês / eventos / visitas)
# ==========================================================
def exibir_por_mes(df: pd.DataFrame, titulo: str, data_col: str = "DATA"):
    ...
    for i, _ in enumerate(MESES, start=1):
        with tabs[i - 1]:
            ...
            # formata DATA (para não aparecer 00:00:00)
            if data_col in dfm.columns:
                dfm[data_col] = dfm["_DATA_DT"].dt.strftime("%d/%m/%Y")
...
def exibir_eventos_por_mes(df_eventos: pd.DataFrame):
    """
    EVENTOS:
    - por mês (DATA)
    - mostra GUARNIÇÃO
    - NÃO mostra PRIORIDADE
    - se PRIORIDADE == SIM -> linha azul
    - LOCAL e PROCEDIMENTO quebram linha e expandem verticalmente (HTML)
    """
    st.markdown("<div class='center' style='font-weight:600;'>EVENTOS</div>", unsafe_allow_html=True)

    if df_eventos is None or df_eventos.empty:
        st.info("Sem registros.")
        return

    df = df_eventos.copy().dropna(axis=1, how="all").dropna(how="all")
    df = df.loc[:, [c for c in df.columns if not _is_unnamed(c)]].copy()
    df.columns = df.columns.astype(str).str.strip().str.upper()
    ...
    for i, _ in enumerate(MESES, start=1):
        with tabs[i - 1]:
            dfm_full = df_ok[df_ok["_MES_NUM"] == i].copy()
            if dfm_full.empty:
                st.info("Sem registros neste mês.")
                continue

            # PRIORIDADE (pinta linha azul se SIM)
            if "PRIORIDADE" in dfm_full.columns:
                mask = (
                    dfm_full["PRIORIDADE"]
                    .fillna("")
                    .astype(str)
                ...
                mask = pd.Series([False] * len(dfm_full), index=dfm_full.index)

            dfm_disp = dfm_full.drop(columns=["_MES_NUM"], errors="ignore").copy()

            # DATA formatada
            if "DATA" in dfm_disp.columns:
                dfm_disp["DATA"] = dfm_disp["_DATA_DT"].dt.strftime("%d/%m/%Y")

            # remove colunas auxiliares e PRIORIDADE (não exibir)
            dfm_disp = dfm_disp.drop(columns=["_DATA_DT"], errors="ignore")
            dfm_disp = dfm_disp.drop(columns=["PRIORIDADE"], errors="ignore").reset_index(drop=True)
            mask_reset = mask.reset_index(drop=True)

            wrap_left = {"LOCAL", "PROCEDIMENTO"}
            _render_html_table(dfm_disp, highlight_mask=mask_reset, wrap_left_cols=wrap_left)
...
def main():
    ...
    # ======================================================
    # PRIMEIRO: DETERMINAÇÕES E OBSERVAÇÕES
    # ======================================================
    st.markdown("<hr/>", unsafe_allow_html=True)
    st.markdown("<div class='center' style='font-weight:700; font-size:18px;'>DETERMINAÇÕES E OBSERVAÇÕES</div>", unsafe_allow_html=True)
